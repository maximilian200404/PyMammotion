...
class DeviceHandle:
    ...
    def _current_report_timestamp(self) -> int | None:
        """Return the latest applied report timestamp from the current device snapshot."""
    ...
    async def on_raw_message(self, payload: bytes, transport_type: TransportType = TransportType.CLOUD_ALIYUN) -> None:
        ...
        Steps:
          ...
          3. Ignore stale report_data messages whose sys_time_stamp is older
             than the current live snapshot
          4. Apply LubaMsg to state via StateReducer
          5. Update DeviceStateMachine and emit the new snapshot
          6. Route LubaMsg to broker for request/response correlation
        """
